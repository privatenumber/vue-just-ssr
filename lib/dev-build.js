...
function createClientBuild(state, clientConfig) {
	...
	function warmCache(assets) {
		for (const assetId in assets) {
			...
			const source = assets[assetId].source();
			const etag = getETag(source);
			etagCache.set(fsPath, etag);
		}
	}
	...
	const middlewares = {
		devMiddleware({url, headers}, response) {
			...
			if (ifNoneMatch) {
				const cacheHit = etagCache.get(filename);
				if (cacheHit === ifNoneMatch) {
			...
			response.send = function (result) {
				const etag = etagCache.get(filename);
	...
	const onDone = stats => {
		warmCache(stats.compilation.assets);
